...
async def analyze_vocal(file: UploadFile = File(...)):
    try:
        ...
        # Universal Isolation Pass: Separate core vocals from acoustic arrangements
        y_vocals = librosa.effects.harmonic(y_raw, margin=4.0)
        y_vocals = librosa.effects.preemphasis(y_vocals)
        
        # Energy Floor Gate: Calculate active amplitude envelopes to skip silence pads
        ...
        if not np.any(active_frames):
            return {"success": False, "error": "Vocal stream density too faint to map parameters."}

        # Core Fundamental Pitch Modeling (Probabilistic Tracking Engine)
        ...
        if len(f0_clean) < 20:
            return {"success": False, "error": "Insufficient continuous pitch fragments isolated."}

        # Pitch Velocity Extraction (Detects instantaneous geometric tuning steps)
        pitch_velocity = np.abs(np.diff(f0_clean))
        clamped_snaps = np.sum(pitch_velocity > 30)
        total_transitions = len(pitch_velocity)
        
        # Harmonicity Texture Coefficient (Measures natural biological vocal fry)
        ...
        # Scoring Calibration Loop Matricies
